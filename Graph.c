...
Graph* GraphCopy(const Graph* g) {
  ...
  // TO BE COMPLETED !!
  unsigned int nVertices = g->numVertices;
  
  if (g->isComplete){
    
    return GraphCreateComplete(nVertices, g->isDigraph);
  }
  else{
    // COMPLETAR
    // Criar um grafo novo com o mesmo numero de vertices
    Graph* g2 = GraphCreate(nVertices, g->isDigraph, g->isWeighted);
    
    // Percorrer a lista de vertices do grafo original
    ...
    assert(GraphCheckInvariants(g2));
    return g2;
  }
  return NULL;
}
...
static unsigned int _GetMaxDegree(const Graph* g) {
  ...
  ListMoveToHead(vertices);
  unsigned int i = 0;
  for (; i < GraphGetNumVertices(g); ListMoveToNext(vertices), i++) {
    struct _Vertex* v = ListGetCurrentItem(vertices); 
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
  ...
  for (unsigned int i = 0; i < ListGetSize(edges); ListMoveToNext(edges), i++) {
    struct _Edge* e = ListGetCurrentItem(edges);
    if (e->adjVertex == w) {
      free(e);
  ...
  // no caso da aresta nao existir retorna 0
  if (count == 0) return 0;

  // ir para o vertice
  ListMove(g->verticesList, w);
  struct _Vertex* vertex_w = ListGetCurrentItem(g->verticesList);
  edges = vertex_w->edgesList;
...
int GraphCheckInvariants(const Graph* g) {
  ...
  return (ListGetSize(g->verticesList) == GraphGetNumVertices(g)) &&
